...
def gobangwin():
    win = GraphWin("this is a gobang game", 320, 320)  # ÖÆ×÷21x21µÄÆåÅÌ
...
def main():
    ...
    while g == 0:
        ...
        if change % 2 == 1:
            ai.go(chessboard)
            pos = ai.candidate_list[-1]
            print("AI pos: " + str(pos))

            if pos in list3:
                message = Text(Point(200, 200), "不可用的位置" + str(pos[0]) + "," + str(pos[1]))
                # print(ai.history_status)
                # print(chessboard)
                message.draw(win)
                g = 1
            ...
            piece = Circle(Point(GRID_WIDTH * (pos[0] + 1), GRID_WIDTH * (pos[1] + 1)), 8)
            piece.setFill('white')
            piece.draw(win)
